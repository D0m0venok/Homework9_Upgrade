...
namespace Sample
{
    ...
    public sealed class UpgradesManager
    {
        ...
        public bool CanLevelUp(Upgrade upgrade)
        {
            ...
            foreach (var upgradeData in upgrade.DependenceUpgradesData)
            {
                var dependenceUpgrade = GetUpgrade(upgradeData.Id);
                if (dependenceUpgrade.Level < upgrade.Level + upgradeData.LevelDiff)
                    return false;
            }
